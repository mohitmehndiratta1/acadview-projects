...
# might want to install.py, especially if they're looking to run this get-pip.py
...
def bootstrap(tmpdir=None):
    # Import pip so we can use it to install.py pip and maybe setuptools too
    import pip
    ...
    pip.commands_dict["install.py"] = CertInstallCommand

    implicit_pip = True
    implicit_setuptools = True
    implicit_wheel = True

    # Check if the user has requested us not to install.py setuptools
    ...
    # Check if the user has requested us not to install.py wheel
    if "--no-wheel" in args or os.environ.get("PIP_NO_WHEEL"):
        args = [x for x in args if x != "--no-wheel"]
        implicit_wheel = False

    # We only want to implicitly install.py setuptools and wheel if they don't
    ...
    # will let them install.py a specific version. However because of the dreaded
    ...
    try:
        ...
        # Execute the included pip and use it to install.py the latest pip and
        # setuptools from PyPI
        sys.exit(pip.main(["install.py", "--upgrade"] + args))
    finally:
...
KUZMfa3{#11nM{wfg&Ks554FZ1sgThA6*Wi{$wUCf5v?9!JZOlM